...
if(isset($forfield) && $forfield != '' && $focus->popup_type != 'detailview') {
	...
	// columns to retrieve
	$columns = array(
		'plate', 'chassis', 'make', 'model', 'vehicle_type');
	$popupValues = array();
	
	$result = $adb->pquery(
		"SELECT ".implode(', ', $columns)."
		FROM {$table_prefix}_vehicles 
		WHERE {$table_prefix}_vehicles.vehiclesid = ?", 
		array($entity_id)
	); 
	...
	$fnName = 'set_return_vehicle_information';
	$argsKeys = array(
			'plate', 'chassis', 'make', 'model', 'vehicle_type'
		);
	...
	// html values
	$value = '<a href="index.php?module=Vehicle&action=DetailView&record='.$entity_id.'&parenttab=Marketing" />';
