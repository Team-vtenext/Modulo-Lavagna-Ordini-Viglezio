...
function update_inventory_row($element, $user)
{
    ...
    if (!$isAdmin) {
        ...
        if ($resEmployee && $adb->num_rows($resEmployee) > 0) {
            ...
            return "Nessun utente con questa carta";
        }
    }
    ...
    $res = $adb->pquery($sql, array($employeesid, $lineItemId));
        
    if ($res && $adb->num_rows($res) > 0) {
        $salesOrder_lineId = $adb->query_result($res, 0, 'salesorder_lineid');

        if ($salesOrder_lineId != $lineItemId) {
            return "Altra lavorazione aperta per questo utente";
        } else {
            $focus = CRMEntity::getInstance('Timesheets');
            $focus->retrieve_entity_info_no_html($adb->query_result($res, 0, 'timesheetsid'), 'Timesheets');
            $focus->column_fields["end_time"] = date("H:i");
            $focus->column_fields['timesheet_status'] = 'Closed';
            $focus->mode = 'edit';
            $focus->save('Timesheets');

            return 'Lavorazione Terminata';
        }
    } else {
        $sql = 'select productid, servicename, salesorder_no FROM ' . $table_prefix . '_inventoryproductrel 
			INNER JOIN ' . $table_prefix . '_service ON ' . $table_prefix . '_service.serviceid = ' . $table_prefix . '_inventoryproductrel.productid
			INNER JOIN ' . $table_prefix . '_salesorder ON ' . $table_prefix . '_salesorder.salesorderid = ' . $table_prefix . '_inventoryproductrel.id
			WHERE id = ? AND timesheet_id = ?';

        $res2 = $adb->pquery($sql, array($crmid, $lineItemId));

        if ($res2 && $adb->num_rows($res2) > 0) {
            $productid = $adb->query_result($res2, 0, 'productid');
            $servicename = $adb->query_result($res2, 0, 'servicename');
            $salesorder_no = $adb->query_result($res2, 0, 'salesorder_no');
            ...
            $focus = CRMEntity::getInstance('Timesheets');
            $focus->column_fields["timesheet_name"] = $servicename . ' ' . $salesorder_no;
            $focus->column_fields["service_id"] = $productid;
            ...
            $focus->column_fields['salesorder_lineid'] =  $lineItemId;
            $focus->column_fields["employee_id"] = $employeesid;
            $focus->column_fields["salesorder_id"] = $crmid;
            $focus->column_fields["assigned_user_id"] = $user->id;
            $focus->save('Timesheets');
    
            return "Nuova Lavorazione Creata";
        }
    }
        
    if ($autoClose == 1) {
        $sql = "SELECT * FROM " . $table_prefix . "_timesheets WHERE salesorder_id = ? AND timesheet_status != 'Closed'";
        $res = $adb->pquery($sql, array($crmid));
        
        if ($res && $adb->num_rows($res) == 0) {
            $sql = "UPDATE " . $table_prefix . "_salesorder SET sostatus = 'Delivered' WHERE salesorderid = ?";
